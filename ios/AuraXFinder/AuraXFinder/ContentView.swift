...
private struct WebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        configuration.userContentController.add(context.coordinator.audio, name: "AuraXAudio")
    ...
    static func dismantleUIView(_ uiView: WKWebView, coordinator: Coordinator) {
        ...
        uiView.configuration.userContentController.removeScriptMessageHandler(forName: "AuraXAudio")
        uiView.stopLoading()
    ...
    final class Coordinator: NSObject, WKUIDelegate, WKNavigationDelegate {
        ...
        init(allowedHost: String?) {
            audio = AudioReactiveController(host: allowedHost ?? "")
        }

        func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction,
                     decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
            guard navigationAction.targetFrame?.isMainFrame != false else { decisionHandler(.allow); return }
            guard let url = navigationAction.request.url, url.scheme == "http", let host = url.host,
                  AudioReactiveController.isLocalIP(host), url.port == nil || url.port == 80 else {
                decisionHandler(.cancel)
                if let url = navigationAction.request.url, ["http", "https"].contains(url.scheme ?? "") { UIApplication.shared.open(url) }
                return
            }
            audio.stop()
            audio.allowedHost = host
        ...
        func webView(
            ...
            guard frame.isMainFrame, origin.protocol == "http", origin.host == audio.allowedHost, type == .microphone else {
